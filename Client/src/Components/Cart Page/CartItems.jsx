  ...
  <div className="flex justify-between items-start gap-3 sm:gap-4">
    <div className="space-y-1">
      <Link to={`/products/${product._id}`}>
        <h3 className="font-serif text-[#322619] font-bold text-base sm:text-lg hover:text-[#B58D2F] transition-colors line-clamp-2 sm:line-clamp-1">
          {product.title}
        </h3>
      </Link>

      <div className="flex items-center gap-2">
        <HiOutlineBadgeCheck className="text-[#B58D2F] text-base sm:text-lg" />
        <span className="text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.2em] text-[#544231]/60">
    ...
    <button
      ...
      className="p-2.5 sm:p-3 text-[#544231]/40 hover:text-[#A34343] hover:bg-red-50 rounded-full transition-all"
    >
      <IoTrashOutline size={20} className="sm:hidden" />
      <IoTrashOutline size={22} className="hidden sm:block" />
    </button>
  </div>
  ...
  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
    ...
    <div className="flex items-center bg-[#F9F6F0] border border-[#EDE3D2] p-1 rounded-full">
      <button
        ...
        disabled={quantity <= 1}
        className="w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center rounded-full bg-white text-[#322619] shadow-sm hover:bg-[#B58D2F] hover:text-white transition-all disabled:opacity-20"
      >
        –
      </button>

      <span className="w-9 sm:w-10 text-center text-sm font-bold text-[#322619]">
        {quantity}
      </span>
    ...
    <div className="text-right w-full sm:w-auto">
      <div className="flex justify-end items-baseline gap-2">
        <span className="text-[#544231]/30 text-xs line-through">
          ₹{(product.oldprice * quantity).toFixed(0)}
        </span>
        <span className="text-[#322619] font-black text-xl sm:text-2xl tracking-tighter">
          ₹{(product.price * quantity).toFixed(0)}
        </span>
      </div>

      <p className="text-[9px] sm:text-[10px] font-bold text-emerald-600 uppercase tracking-widest mt-1">
