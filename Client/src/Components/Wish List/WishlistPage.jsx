...
function WishlistPage() {
  ...
  return (
    <section className="min-h-screen bg-[#FDFBF7] py-6 sm:py-10">
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-10">
          ...
          <main className="w-full lg:w-3/4 order-1 lg:order-2">
            <div className="bg-white rounded-3xl border border-stone-200 shadow-sm overflow-hidden">
              ...
              {/* HEADER */}
              <header className="px-5 py-6 sm:px-8 sm:py-8 border-b border-stone-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                  <h1 className="text-2xl sm:text-3xl md:text-4xl font-serif font-bold text-[#1A3A32]">
                    My Saved{" "}
                    <span className="italic font-normal text-[#C5A059]">
                      Treasures
                    </span>
                  </h1>
                  <p className="text-xs uppercase tracking-widest text-stone-400 mt-1 font-semibold">
              ...
              <div className="p-4 sm:p-6 md:p-8">
                {wishlistItems.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 max-h-[70vh] overflow-y-auto pr-1 no-scrollbar">
                    <AnimatePresence>
                      {wishlistItems.map((item) => {
                        ...
                        return (
                          <motion.div
                            ...
                            className="relative flex gap-4 bg-white rounded-2xl border border-stone-100 p-4 hover:border-[#C5A059]/40 hover:shadow-lg transition"
                          >
                            {/* REMOVE */}
                            <button
                              onClick={() => handleRemove(product._id)}
                              className="absolute top-3 right-3 p-1.5 rounded-full bg-white border text-stone-400 hover:text-red-500 hover:scale-110 transition"
                            >
                            ...
                            <Link
                              ...
                              className="w-20 h-20 sm:w-28 sm:h-28 rounded-xl overflow-hidden bg-[#FDFBF7] border flex-shrink-0"
                            >
                              <img
                                src={
                            ...
                            <div className="flex flex-col justify-between min-w-0">
                              ...
                              <Link
                                to={`/products/${product._id}`}
                                className="mt-2 inline-flex items-center gap-1 text-[10px] uppercase tracking-widest font-bold text-[#1A3A32] hover:text-[#C5A059]"
                              >
                                View Piece
                  ...
                  <div className="py-20 text-center flex flex-col items-center">
                    ...
                    <h3 className="text-xl sm:text-2xl font-serif font-bold text-[#1A3A32]">
