...
function AddAddress() {
  ...
  className={`relative flex flex-col sm:flex-row items-start gap-4 sm:gap-5 
  p-4 sm:p-6 rounded-2xl sm:rounded-[2rem] border-2 transition-all cursor-pointer
  focus:outline-none focus:ring-2 focus:ring-[#B58D2F] focus:ring-offset-4 ${
    selectedAddress?._id === addr._id
      ? "border-[#B58D2F] bg-[#FDFBF7] shadow-lg shadow-[#B58D2F]/5 sm:scale-[1.01]"
...
>
  ...
  <div className="flex-1 w-full">
    <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-2">
      <span className="font-serif text-lg sm:text-xl font-bold text-[#322619]">
        {addr.fullName}
      </span>

      <div className="flex gap-2">
        {addr.isDefaultBilling && (
          <span className="text-[9px] bg-[#B58D2F] text-white px-3 py-1 rounded-full font-black uppercase tracking-widest">
            Default
          </span>
        )}
      </div>
    ...
    <p className="text-sm text-[#544231] font-bold mb-1 sm:mb-2">
      {addr.phoneNumber}
    </p>

    <p className="text-sm text-[#544231]/70 leading-relaxed font-medium">
      {addr.address}, {addr.city}, {addr.state} —{" "}
      <span className="font-bold text-[#322619]">{addr.pin}</span>
    </p>
    ...
    {selectedAddress?._id === addr._id && (
      <Button
        ...
        !bg-[#322619] !text-[#F9F6F0] !px-8 sm:!px-10 !py-3.5
        !rounded-full !font-bold !capitalize !text-sm
        !shadow-xl hover:!bg-[#B58D2F] !transition-all"
        onClick={(e) => {
          e.stopPropagation();
          handlePlaceOrder();
        }}
  ...
  {/* ACTIONS */}
  <div
    className="flex flex-row sm:flex-col gap-2 sm:gap-3
    border-t sm:border-t-0 sm:border-l border-dashed border-[#EDE3D2]
    pt-3 sm:pt-0 sm:pl-4 w-full sm:w-auto justify-end"
  >
    <button
      type="button"
      onClick={(e) => {
      ...
      className="text-[#544231]/40 hover:text-[#B58D2F] p-2 transition-colors
      bg-[#F9F6F0] rounded-full"
      aria-label="Edit"
    >
      <MdModeEdit size={18} />
    </button>
    ...
    <button
      ...
      className="text-[#544231]/40 hover:text-red-500 p-2 transition-colors
      bg-[#F9F6F0] rounded-full"
      aria-label="Delete"
    >
      <MdDelete size={18} />
                  ...
                  <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    ...
                    <div className="md:col-span-2 flex items-center gap-6 mt-6">
                      <Button
                        type="submit"
                        variant="contained"
                        className="!bg-[#322619] !text-[#F9F6F0] !px-6 !py-4 !rounded-full !font-bold !capitalize !shadow-xl hover:!bg-[#B58D2F] !transition-all"
                        disabled={isSaving}
                      >
