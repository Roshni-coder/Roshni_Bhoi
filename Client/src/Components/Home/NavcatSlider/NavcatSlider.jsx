...
const NavCatSlider = () => {
  ...
  return (
    <div className="bg-[#faf9f6]  border-b border-[#e7ddcf] py-10">
          ...
          {categories.map((category, index) => (
            <SwiperSlide key={index}>
              ...
              >
                ...
                <div className="relative w-20 h-20 sm:w-32 sm:h-32 rounded flex items-center justify-center transition-all duration-500">
                  
                  {/* Inner Gold Ring (Visible on Hover) */}
                  <div className="absolute inset-0 rounded border border-[#d4af37]/0 group-hover:border-[#d4af37]/60 group-hover:scale-110 transition-all duration-500" />
                  
                  {/* Outer Frame */}
                  <div className="w-[90%] h-[90%] rounded p-[3px] bg-white border border-[#e7ddcf] group-hover:border-[#d4af37] shadow-sm transition-all duration-500 overflow-hidden">
                    <img
                      src={getCategoryImageUrl(category)}
                      alt={category.categoryname}
                      className="w-full h-full object-cover rounded filter grayscale-[30%] group-hover:grayscale-0 transition-all duration-700 group-hover:scale-110"
                    />
                  </div>
