...
function OrderSummery() {
  ...
  const checkoutHandler = async () => {
    ...
    try {
      const {
        data: { key },
      } = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/getkey`);
      const {
        data: { order },
      } = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/checkout`, {
        amount: totalAmount,
      });
      ...
      await axios.delete(
        `${import.meta.env.VITE_BACKEND_URL}/api/auth/clear-cart`,
        { headers: { Authorization: `Bearer ${token}` } }
      );

      // Optionally also clear local state
      setCartItems([]);
